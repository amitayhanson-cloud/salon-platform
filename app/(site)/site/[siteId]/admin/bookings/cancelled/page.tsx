...
import { fromYYYYMMDD, toYYYYMMDD } from "@/lib/calendarUtils";
...
export default function CancelledBookingsPage() {
  ...
  const filteredBookings = useMemo(() => {
    return archiveBookings.filter((b) => {
      const d = b.date || "";
      if (!d) return false;
  ...
  }, [archiveBookings, dateFrom, dateTo]);
    ...
    <div className="min-h-screen" dir="rtl">
      <div className="max-w-7xl mx-auto">
        <div className="relative mb-6">
          ...
          <AdminPageHero
            title="תורים שבוטלו"
            subtitle="רשומות לאחר ארכיון (ביטול מהמערכת, מ־WhatsApp או מדף ההזמנה). מופיעות גם בכרטיס לקוח. אין מחיקה כאן — התור כבר הוסר מהיומן הפעיל."
            className="flex-1"
          />
        </div>
        {error && (
            ...
            <div className="p-12 text-center">
              <p className="text-slate-500 text-sm">
                {archiveBookings.length === 0
                  ? "אין תורים שבוטלו"
                  : "אין תורים שבוטלו בטווח התאריכים שנבחר"}
              </p>
            </div>
          ) : (
            <>
